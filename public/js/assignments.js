...
function buildPage() {
  ...
  main.innerHTML = '<div class="page-header">'
    ...
    + '<button class="btn-primary" onclick="saveAssignment()">+ Add Assignment</button>'
    + '</div>'
...
function saveAssignment() {
  ...
  if (Auth.isGuest()) {
    ...
    clearForm(); renderAssignments(); showToast('Assignment added!'); return;
  }

  api.post('/api/assignments', { title: title, subject: subject, due_date: due, priority: priority, notes: notes })
    .then(function (r) {
      if (!r || r.error) { showToast(r ? r.error : 'Error'); return; }
      clearForm(); renderAssignments(); showToast('Assignment added!');
    });
}

function clearForm() {
  ['a-title','a-subject','a-due','a-notes'].forEach(function (id) { document.getElementById(id).value = ''; });
}
...
function toggleDone(id) {
  if (Auth.isGuest()) {
    ...
    GuestDB.set('assignments', list);
    renderAssignments(); return;
  }
  api.patch('/api/assignments/' + id + '/toggle').then(renderAssignments);
}
...
function deleteAssignment(id) {
  ...
  api.delete('/api/assignments/' + id).then(renderAssignments);
}
...
function renderAssignments() {
  ...
  api.get(url).then(function (r) { renderList(Array.isArray(r) ? r : []); });
}

function renderList(list) {
  var el = document.getElementById('assign-list');
  if (!list.length) {
    el.innerHTML = '<div class="empty-state"><div class="icon">✅</div><h3>Nothing here!</h3><p>All clear.</p></div>'; return;
  }
  el.innerHTML = list.map(function (a) {
    ...
    return '<div class="assign-item' + (a.done ? ' done' : '') + '">'
      ...
      + '<div class="assign-title" style="' + (a.done ? 'text-decoration:line-through' : '') + '">' + a.title + '</div>'
